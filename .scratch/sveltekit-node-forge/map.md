...
- [Specify the Node release and environment contract](issues/02-specify-release-and-environment-contract.md) — Build each immutable release in place with Node 22.x and pinned pnpm, pruned production dependencies, and a validated per-site runtime `.env`; staging and production deliberately share only the read-only Craft upstream credentials.
