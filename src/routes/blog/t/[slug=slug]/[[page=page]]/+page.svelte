<script lang="ts">
  ...
  import { type EntryType_BlogSingleFragment, type EntryType_TopicFragment } from "$graphql/graphql";

  let { data }: PageProps = $props();
  const entryCount = data.entryCount ?? 1;
  const totalPages = data.totalPages ?? 1;
  let topicEntry = $derived(getFirstEntry(data.topicEntry) as EntryType_TopicFragment);
  let entries = $derived(data.entries as EntryType_BlogSingleFragment[]);
  let page = $derived(data.page);

  $effect(() => {
    if (browser && page === 1 && topicEntry?.uri) {
      replaceState(`/${topicEntry.uri}`, {});
    }
  });
  ...
  let letters = $derived(splitTextIntoDivs(topicEntry?.title, "is-fadeIn lg:is-blurInLeftDown", "$"));
</script>

{#if topicEntry?.seomatic}
  <Seo seo={topicEntry.seomatic} />
{/if}
...
{#if page === 1}
  ...
  {/if}
  {#if topicEntry.description}
    <RichText className={cc.text} html={topicEntry.description} data-waypoint-target />
  {/if}
{:else}
